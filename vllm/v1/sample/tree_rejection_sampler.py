...
class TreeRejectionSampler(nn.Module):
    ...
    def forward(
        ...
        col_indices = torch.cumsum(path_masks, dim=1) - 1
        row_indices = batch_indices.unsqueeze(1).expand((-1, draft_tree_size))
        scatter_rows = row_indices[path_masks]
        scatter_cols = col_indices[path_masks]
        accepted_draft_token_ids = draft_token_ids[path_masks]
        output_token_ids[scatter_rows, scatter_cols] = accepted_draft_token_ids
        ...
        output_token_ids[
            batch_indices,
            accepted_path_levels] = bonus_sampler_output.sampled_token_ids.view(
                -1)

        if num_req > num_tree_decodes:
            # In some cases, we may have leftover requests with 0 draft tokens.
            # Sample a token for each.
            bonus_sampler_output = self.main_sampler(
                logits=target_logits[num_tree_logits:],
                sampling_metadata=sampling_metadata,
            )
            output_token_ids[num_tree_decodes:,
                             0] = bonus_sampler_output.sampled_token_ids.view(
                                 -1)

        accepted_indices = [torch.where(row)[0] for row in path_masks]
        return output_token_ids, accepted_indices
