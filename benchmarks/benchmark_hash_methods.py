...
class HashBenchmark:
    ...
    def run_benchmark(self, iterations: int = 10000):
        ...
        # Calculate data size
        data_size_bytes = len(str(self.test_data).encode("utf-8"))

        # Calculate throughput (MB/s)
        sha256_throughput = (data_size_bytes / (1024 * 1024)) / sha256_time
        sha256_cbor_throughput = (data_size_bytes / (1024 * 1024)) / sha256_cbor_time
        builtin_throughput = (data_size_bytes / (1024 * 1024)) / builtin_time
        ...
        print(
            f"  SHA256:        {sha256_time * 1e6:8.2f} ± \
            {sha256_std * 1e6:6.2f} μs  ({sha256_throughput:6.2f} MB/s)"
        )
        print(
            f"  SHA256_CBOR:        {sha256_cbor_time * 1e6:8.2f} ± \
            {sha256_cbor_std * 1e6:6.2f} μs  ({sha256_cbor_throughput:6.2f} MB/s)"
        )
        print(
            f"  Built-in hash: {builtin_time * 1e6:8.2f} ± \
            {builtin_std * 1e6:6.2f} μs  ({builtin_throughput:6.2f} MB/s)"
        )
